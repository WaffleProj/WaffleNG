    ...
    )
{
    ...
    else if (!lpstLibrary[dwLibrary].lpstFunction[dwFunction].lpDetour)
    {
        return bDetour;
    }
    else if (!lpstLibrary[dwLibrary].lpstFunction[dwFunction].lpSource)
    {
        return bDetour;
    }

    //MessageBox(0, lpstLibrary[dwLibrary].lpstFunction[dwFunction].lpszFunction, lpstLibrary[dwLibrary].lpszLibrary, 0);
    if (!bDetour)
    {
        if (!WaffleJumpDetection(lpstLibrary[dwLibrary].lpstFunction[dwFunction].lpSource))
        {
            bDetour = WaffleInlineDetour(lpstLibrary[dwLibrary].lpstFunction[dwFunction].lpSource);
        }
    }
    if (!bDetour)
    {
        bDetour = WaffleExceptionDetour(lpstLibrary[dwLibrary].lpstFunction[dwFunction].lpSource);
    }
    return bDetour;
}
